...
//									g.AddTaskItem(new JSGantt.TaskItem(11, '安卓端UI设计',  '3/17/2016',  '3/21/2016', '74BDFF', '', 0, '按时大大',    100, 0, 0, 1,'','', '延时完成'));
//									g.AddTaskItem(new JSGantt.TaskItem(1,   '易管理web端UI设计', '3/24/2016', '4/1/2016', '74BDFF', '', 0, 'devil',     80, 0, 0, 1,'','', '进行中'));
//									
//								    g.AddTaskItem(new JSGantt.TaskItem(2,   '易管理web端前端实现', '', '', '74BDFF', '', 0, 'devil',     0, 1, 0, 1,'','', '进行中'));
//									g.AddTaskItem(new JSGantt.TaskItem(21,   '甘特图', '3/30/2016', '4/8/2016', '74BDFF', '', 0, 'devil',     30, 0, 2, 1,'','', '进行中'));
//									g.AddTaskItem(new JSGantt.TaskItem(22,  '登录，修改密码',     '4/8/2016', '4/10/2016', '74BDFF', '', 0, 'Brian',    30, 0, 2, 1, '','', '进行中'));
//								
//						
//								    g.AddTaskItem(new JSGantt.TaskItem(3,   '后端接口编写',      '',          '',          'ff0000', '', 0, '松风阁',     0, 1, 0, 1 , '', '', ''));
//								    g.AddTaskItem(new JSGantt.TaskItem(31,  '日报接口',     '3/25/2016', '3/28/2016', '74BDFF', '', 0, '松风阁',    100, 0, 3, 1, '','', '已完成'));
//								    g.AddTaskItem(new JSGantt.TaskItem(32,  '评论功能', '3/29/2016', '3/30/2016', '74BDFF', '', 0, 'Shlomy',   100, 0, 3, 1, '', '', '延期完成'));
//								    g.AddTaskItem(new JSGantt.TaskItem(33,  'IOS端接口',      '',          '',          '00ff00', '', 0, '松风阁',  40, 1, 3, 1));
//								    g.AddTaskItem(new JSGantt.TaskItem(332, '项目页面接口编写',     '3/30/2016',  '4/11/2016', '74BDFF', '', 0, '松风阁',    20, 0, 33, 1, '', '', '进行中'));
//								    g.AddTaskItem(new JSGantt.TaskItem(333, '工作台页面接口编写',  '4/11/2016',  '4/20/2016', '74BDFF', '', 0, '松风阁',    0, 0, 33, 1,'','', '未开始'));
//								    g.AddTaskItem(new JSGantt.TaskItem(4,  '安卓端APP',       '',          '',          '99ff00', '', 0, 'Anybody',  0, 1, 0, 1));
//								    g.AddTaskItem(new JSGantt.TaskItem(41, '安卓端APP前端实现',  '4/11/2016',  '4/30/2016', '74BDFF', '', 0, '松风阁',    0, 0, 4, 1,'','', '未开始'));
								...
								$.each(task,function(i,item){
									...
									if(item.ChildTask.length > 0){
										...
										$.each(item.ChildTask,function(j,childitem){
											if(childitem != undefined && childitem.ftId == item.tId){
												console.log(childitem.tStartDate);
												var res = childitem.tStartDate.replace( /(\d{4})\/(\d{2})\/(\d{2})/, "$2/$3/$1" );
												console.log(res);
								...
								*/
								...
								var asd = $('#rightside').parent().width();
								console.log(asd);
								$('#rightside').css('width', asd);
			...
			$.post("/UserManage.aspx?method=UserList", tData).then(function(result) {
				...
				switch (d.Result) {
					...
					case "1":
						...
						var uid = $(".participant_user_result").attr("data-uid")
						if(uid){
							uid = JSON.parse(uid);
		...
		/*$(document).on("click", ".smember_btn_default", function(e){//添加负责人参与人确认
			...
			switch(type){
				...
				case "principal_task"://任务负责人
					var uid = $("[name=principal_user]:checked").val();
					var name = $("[name=principal_user]:checked").attr("data-name");
					uid && $(".project_detail_content .task-create-principle-inner").attr("data-uid",uid);
					name && $(".project_detail_content .task-create-principle-label").html(name+"负责");
					$(".smember_container").hide();
					break;
				case "participant_task"://任务参与人
					...
					name = name.substring(0,name.length-1);
					uid && $(".project_detail_content .participant_taskuser_result").attr("data-uid",JSON.stringify(uid));
					$(".project_detail_content .participant_taskuser_result").html(name);
					$(".smember_container").hide();
					break;
				case "project_add_user"://添加成员 TODO
					...
					$.each($("[name=participant_user]:checked"),function(index,item){
						var uid = $(this).val();
						ProjectUserList.push({teamId:userData.teamId,pId:pid,uId:uid,uRole:2});//参与人
					});
					var data = {
						token: localStorage.token,
						uid: userData.uId,
		...
		$(document).on("click", ".project_item_container .project_create_btn", function(e) {//新建项目确认
			...
			if(!pName || !pDescription || !principal_uid){
				//alert("内容不能为空");
				return;
			}
			var data = {
			...
			$.post("/ProjectManage.aspx?method=GetProjectUser", tData).then(function(result) {
				...
				switch (d.Result) {
					...
					case "1":
						...
						$.each(userlist,function(index,item){
							//if(item.uId == userData.uId) return true;
							var partuser = $(".project_detail_content .participant_taskuser_result").attr("data-uid");
							if(partuser && partuser.indexOf(item.uId) != -1) return;
							var str = '<label><input name="principal_user" type="radio" value="'+ item.uId + '" data-name="'+ item.nickName +'"> '+ item.nickName +' </label>';
							$(".smember_list_parent").append(str);
						});
						var uid = $(".project_detail_content .task-create-principle-inner").attr("data-uid");
						uid && $("input[value="+ uid +"]").attr("checked","checked");*/
		...
		$(document).on("click", ".project_detail_content .task-create-bottom .btn-sure", function(e){ //添加任务 确认
			...
			if(!tName || !tDescribe || !principal_uid){
				//alert("内容不能为空");
				return;
			}
			var data = {
